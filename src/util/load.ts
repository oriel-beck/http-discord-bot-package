...
import { AutocompleteInteractionController, ComponentInteractionController } from "../controllers/controller.js";
import type { Logger } from "pino";
import SuperMap from "@thunder04/supermap";

export async function walkRoutes<T extends AnyController>(router: findMyWay.Instance<findMyWay.HTTPVersion.V1>, folder: string, logger: Logger, store: SuperMap<string, T>, prefix = "") {
    ...
    for (const dirent of files) {
        ...
        if (direntStat.isDirectory()) {
            ...
            router.post(route, function (this: {data: unknown }, req, res, params) { controllerInstance.handler(req, res, params, this.data as never) });
        }
    }
};
