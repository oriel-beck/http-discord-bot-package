...
async function loadCommands(publicKey, router, path, prefix, store) {
    ...
    for (const dirent of await (0, promises_1.readdir)(path, { withFileTypes: true })) {
        if (dirent.isDirectory()) {
            await loadCommands(publicKey, router, (0, path_1.join)(path, dirent.name), (0, util_1.joinRoute)(prefix, dirent.name), store);
        }
        else {
...
async function loadComponents(publicKey, router, path, prefix, store) {
    ...
    for (const dirent of await (0, promises_1.readdir)(path, { withFileTypes: true })) {
        if (dirent.isDirectory()) {
            await loadComponents(publicKey, router, (0, path_1.join)(path, dirent.name), (0, util_1.joinRoute)(prefix, dirent.name), store);
        }
        else {
            // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access
            const controllerInstance = await getController(`file://${(0, path_1.join)(path, dirent.name)}`);
...
async function loadModals(publicKey, router, path, prefix, store) {
    ...
    for (const dirent of await (0, promises_1.readdir)(path, { withFileTypes: true })) {
        if (dirent.isDirectory()) {
            await loadModals(publicKey, router, (0, path_1.join)(path, dirent.name), (0, util_1.joinRoute)(prefix, dirent.name), store);
        }
        else {
            // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access
            const controllerInstance = await getController(`file://${(0, path_1.join)(path, dirent.name)}`);
...
async function loadAutocomplete(publicKey, router, path, prefix, store) {
    ...
    for (const dirent of await (0, promises_1.readdir)(path, { withFileTypes: true })) {
        if (dirent.isDirectory()) {
            await loadAutocomplete(publicKey, router, (0, path_1.join)(path, dirent.name), (0, util_1.joinRoute)(prefix, dirent.name), store);
        }
        else {
            // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access
            const controllerInstance = await getController(`file://${(0, path_1.join)(path, dirent.name)}`);
...
function loadRoute(publicKey, router, route, controller) {
    router.post(route, function (req, res, params) {
        const verified = (0, util_1.verifyRequest)(publicKey, req, this.buffer);
        if (!verified)
...
function genRoute(...parts) {
    let route = (0, util_1.joinRoute)(...parts);
